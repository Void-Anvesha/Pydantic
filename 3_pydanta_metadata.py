...
#Field can specify any constarinits but other than that 
#It can also attach description to the fields(metadata) & using Annotated
    name: Annotated[str,Field(max_length=50,title='Name of the patient',description='Give the name of the patient in less than 50 characters',examples=['Mitish','Amit'])]
    email : EmailStr
    linkedin : AnyUrl
    age:int=Field(gt=0,lt=120)
    #Constraint lag gya koi bhi weight ki value 0 se kam nahi set kar skta
    #strict->throw error when the data type not matchews the field type
